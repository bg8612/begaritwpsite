...
if ( ! class_exists( 'YITH_WooCommerce_Catalog_Mode_Premium' ) ) {
	...
	class YITH_WooCommerce_Catalog_Mode_Premium extends YITH_WooCommerce_Catalog_Mode {
		...
		public function enqueue_premium_scripts_admin() {
			...
			wp_register_script( 'ywctm-admin-premium', yit_load_css_file( YWCTM_ASSETS_URL . 'js/admin-premium.js' ), array( 'jquery' ), YWCTM_VERSION, false );
		...
		public function show_wapo_if_hidden() {
			...
			global $post, $product, $Product_Addon_Display;//phpcs:ignore
			...
			if ( ! empty( $active_form ) && '' !== $active_form['form_id'] ) {
				...
				<div class="ywctm-inquiry-form-wrapper <?php echo ( 'toggle' === $form_style ) ? 'has-toggle' : ''; ?>">
					...
					if ( 'toggle' === $form_style ) {
						...
						echo sprintf( '<%1$s class="ywctm-form-title">%2$s</%1$s>', $title_wrapper, $tab_title ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					?>
		...
		public function get_inquiry_form( $key, $tab ) {
			...
			echo apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_text_before_form' ), $product_id, 'ywctm_text_before_form' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public function get_custom_button_template( $button_id = false, $replaces = 'atc', $is_loop = false ) {
			...
			<div class="ywctm-custom-button-container ywctm-button-<?php echo esc_attr( $button_id ); ?>" data-product_id="<?php echo esc_attr( $product->get_id() ); ?>">
				<?php echo sprintf( '<%1$s class="%2$s" %3$s>%4$s%5$s</%1$s>', $button_type, implode( ' ', $classes ), $button_url, $button_icon, $button_text ); //phpcs:ignore ?>
			</div>
			<?php

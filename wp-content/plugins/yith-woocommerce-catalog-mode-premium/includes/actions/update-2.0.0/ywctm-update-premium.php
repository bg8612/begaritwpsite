...
if ( ! function_exists( 'ywctm_upgrade_2_0_0' ) ) {
	...
	function ywctm_upgrade_2_0_0() {
		...
		if ( 'yes' === (string) get_transient( 'ywctm_update' ) || (string) YWCTM_VERSION === (string) get_transient( 'ywctm_prune_settings' ) ) {
...
if ( ! function_exists( 'ywctm_upgrade_settings_premium' ) ) {
	...
	function ywctm_upgrade_settings_premium( $args ) {
		...
		if ( 'country' === (string) $users_option ) {

			$geolocation_settings = array(
				'action'    => 'yes' === (string) get_option( 'ywctm_hide_countries_reverse' . $vendor_id ) ? 'disable' : 'enable',
				'users'     => 'all',
		...
		// Review settings.
		if ( 'unregistered' === (string) get_option( 'ywctm_disable_review' . $vendor_id ) ) {
		...
		if ( '' === (string) $vendor_id ) {
			...
			if ( 'yes' === (string) get_option( 'ywctm_admin_override' ) ) {

				$exclusion = get_option( 'ywctm_admin_override_exclusion', 'no' );
				$reverse   = get_option( 'ywctm_admin_override_reverse', 'no' );

				if ( 'yes' === (string) $exclusion && 'no' === (string) $reverse ) {
					$option = array(
						'action' => 'enable',
						'target' => 'selection',
					);
				} elseif ( 'yes' === (string) $exclusion && 'yes' === (string) $reverse ) {
					$option = array(
						'action' => 'disable',
						'target' => 'selection',
					);
		...
		if ( 'yes' === (string) get_option( 'ywctm_hide_cart_header' . $vendor_id ) ) {
		...
		if ( 'no' === (string) $hide_loop && 'no' === (string) $hide_single ) {
			$atc_option = array(
				'action' => 'show',
				'where'  => 'all',
				'items'  => 'all',
			...
			switch ( true ) {
				case 'no' === (string) $hide_loop && 'yes' === (string) $hide_single:
					$where = 'product';
					break;
				case 'yes' === (string) $hide_loop && 'no' === (string) $hide_single:
					$where = 'shop';
					break;
				default:
					$where = 'all';
			...
			$atc_option = array(
				'action' => 'yes' === (string) $reverse_atc ? 'show' : 'hide',
				'where'  => $where,
				'items'  => 'yes' === (string) $exclude_atc ? 'exclusion' : 'all',
			);
		}
		...
		if ( 'no' === (string) $hide_price ) {
			...
			$price_option = array(
				'action' => 'yes' === (string) $reverse_price ? 'show' : 'hide',
				'items'  => 'yes' === (string) $exclude_price ? 'exclusion' : 'all',
			);
		}
		...
		if ( '' !== (string) $alternative_text ) {
			...
			if ( ywctm_is_wpml_active() ) {
				...
				foreach ( $languages as $language ) {
					...
					if ( $translated_label !== (string) $alternative_text ) {
		...
		if ( 'yes' === (string) get_option( 'ywctm_custom_button' . $vendor_id ) || 'yes' === (string) get_option( 'ywctm_custom_button_loop' . $vendor_id ) ) {
			...
			$url             = 'generic' !== (string) $button_url_type ? $button_url_type . ':' . $button_url : $button_url;
			...
			if ( ywctm_is_wpml_active() ) {
				...
				foreach ( $languages as $language ) {
					...
					if ( $translated_label !== (string) $button_text ) {
		...
		if ( 'none' === (string) get_option( 'ywctm_inquiry_form_type' . $vendor_id ) ) {
...
if ( ! function_exists( 'ywctm_upgrade_exclusions_premium' ) ) {
	...
	function ywctm_upgrade_exclusions_premium( $args ) {
		...
		foreach ( $button_exclusions as $exclusion ) {
			...
			if ( 'product' === (string) $exclusion['type'] ) {
...
if ( ! function_exists( 'ywctm_create_button_from_exclusions' ) ) {
	...
	function ywctm_create_button_from_exclusions( $transient_id ) {
		...
		$vendor_id       = $args['vendor_id'];
		$post_author     = $args['post_author'];
		$button_text     = '' !== (string) $button['button_text'] ? $button['button_text'] : get_option( 'ywctm_button_text' . $vendor_id );
...
if ( ! function_exists( 'ywctm_assign_custom_button' ) ) {
	 ...
	 * @param integer $button_id Button ID.
...
if ( ! function_exists( 'ywctm_keep_legacy_buttons' ) ) {
	 ...
	 * @author  Alberto Ruggiero <alberto.ruggiero@yithemes.com>
	 */
	function ywctm_keep_legacy_buttons( $value, $post_id, $type ) {

		if ( 'custom_button' === (string) $type && 'custom_button_loop' === (string) $type ) {
...
if ( ! function_exists( 'ywctm_return_legacy_settings' ) ) {
	...
	function ywctm_return_legacy_settings( $settings, $id ) {
		...
		if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_postmeta', $product->get_meta( '_ywctm_exclude_button' ), $post_id, '_ywctm_exclude_button' ) ) {
			return array();
		}

		if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_postmeta', $product->get_meta( '_ywctm_custom_url_enabled' ), $post_id, '_ywctm_custom_url_enabled' ) ) {
			...
			foreach ( $product_cats as $cat_id ) {
				...
				if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_exclude_button', true ), $post_id, $cat_id, '_ywctm_exclude_button' ) ) {
					return array();
				}

				if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_custom_url_enabled', true ), $post_id, $cat_id, '_ywctm_custom_url_enabled' ) ) {
					if ( '' === (string) $button_text ) {
			...
			if ( '' === (string) $protocol ) {
				...
				foreach ( $product_tags as $tag_id ) {
					...
					if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_exclude_button', true ), $post_id, $tag_id, '_ywctm_exclude_button' ) ) {
						return array();
					}

					if ( 'yes' === (string) apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_custom_url_enabled', true ), $post_id, $tag_id, '_ywctm_custom_url_enabled' ) ) {
						if ( '' === (string) $button_text ) {
		...
		$button_url_type = 'generic' === (string) $protocol ? '' : $protocol . ':';
...
if ( ! function_exists( 'ywctm_legacy_button_classes' ) ) {
	 ...
	 *
	 * @param string $classes   Legacy cLasses.

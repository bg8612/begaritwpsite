...
if ( ! class_exists( 'YWCTM_Vendors_Table' ) ) {
	...
	class YWCTM_Vendors_Table {
		...
		public function __construct() {
			...
			if ( ! isset( $_GET['sub_tab'] ) || ( isset( $_GET['sub_tab'] ) && 'exclusions-vendors' !== $_GET['sub_tab'] ) ) { //phpcs:ignore
				return;
			}
		...
		private function print_template( $table, $fields, $message ) {
			...
			<div class="yith-plugin-fw-wp-page-wrapper ywctm-exclusions">
				<div class="wrap">
					...
					<?php if ( empty( $getter['action'] ) || ( 'insert' !== $getter['action'] && 'edit' !== $getter['action'] ) ) : ?>
						<?php
					...
					<?php endif; ?>
					<?php
					if ( ! empty( $getter['action'] ) && ( 'insert' === $getter['action'] || 'edit' === $getter['action'] ) ) :
						...
						<form id="form" method="POST" action="<?php echo esc_url( add_query_arg( $query_args, admin_url( 'admin.php' ) ) ); ?>">
							...
							<input id="<?php echo esc_attr( $getter['action'] ); ?>" name="<?php echo esc_attr( $getter['action'] ); ?>" type="submit" class="<?php echo esc_attr( 'insert' === $getter['action'] ? 'yith-save-button' : 'yith-update-button' ); ?>" value="<?php echo( ( 'insert' === $getter['action'] ) ? esc_html__( 'Add vendor', 'yith-woocommerce-catalog-mode' ) : esc_html__( 'Update vendor', 'yith-woocommerce-catalog-mode' ) ); ?>" />
						</form>
					...
					<?php else : ?>
						...
						<form id="custom-table" method="GET" action="<?php echo esc_attr( $list_url ); ?>">
							...
							<input type="hidden" name="page" value="<?php echo esc_attr( $getter['page'] ); ?>" />
							<input type="hidden" name="tab" value="<?php echo esc_attr( $getter['tab'] ); ?>" />
							<input type="hidden" name="sub_tab" value="<?php echo esc_attr( $getter['sub_tab'] ); ?>" />
							<?php $table->display(); ?>
						</form>
		...
		public function add_options( $current_screen ) {
			...
			if ( ( 'yith-plugins_page_yith_wc_catalog_mode_panel' === $current_screen->id ) && ( isset( $_GET['tab'] ) && 'exclusions' === $_GET['tab'] ) && ( ! isset( $_GET['action'] ) || ( 'edit' !== $_GET['action'] && 'insert' !== $_GET['action'] ) ) ) { //phpcs:ignore

...
if ( ! class_exists( 'YWCTM_Exclusions_Table' ) ) {
	...
	class YWCTM_Exclusions_Table {
						...
						'function_delete' => function () {
							...
							if ( ! empty( $ids ) ) {
								foreach ( $ids as $id ) {
									...
									if ( ( isset( $data[1] ) && 'product' === $data[1] ) || ( isset( $_GET['item_type'] ) && 'product' === $_GET['item_type'] ) ) { //phpcs:ignore
		...
		public function add_options( $current_screen ) {
			...
			if ( ( 'yith-plugins_page_yith_wc_catalog_mode_panel' === $current_screen->id || 'toplevel_page_yith_vendor_ctm_settings' === $current_screen->id ) && ( isset( $_GET['tab'] ) && 'exclusions' === $_GET['tab'] ) && ( ! isset( $_GET['action'] ) || ( 'edit' !== $_GET['action'] && 'insert' !== $_GET['action'] ) ) ) { //phpcs:ignore

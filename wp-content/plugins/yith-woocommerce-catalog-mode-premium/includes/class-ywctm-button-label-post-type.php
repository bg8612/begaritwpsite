...
if ( ! class_exists( 'YWCTM_Button_Label_Post_Type' ) ) {
	...
	class YWCTM_Button_Label_Post_Type {
		...
		public function add_retinaicons( $icons ) {
			...
			$icons['retinaicon-font'] = json_decode( file_get_contents( YWCTM_ASSETS_PATH . 'fonts/retinaicon-font.json' ), true ); //phpcs:ignore
			...
			<div class="ywctm-metabox-wrapper">
				<div class="yith-plugin-ui yith-plugin-fw">
					<?php foreach ( $groups as $key => $group ) : ?>
						<div class="ywctm-settings-wrapper <?php echo esc_attr( 'group1' === $key ? 'is-opened' : '' ); ?>">
					...
					<div class="sticky">
						...
						<div id="submitpost" class="yith-plugin-ui">
							...
							if ( 'auto-draft' !== $post->post_status ) {
								...
								$delete = sprintf( ' <a href="%s" class="button-secondary button-xl">%s</a>', get_delete_post_link( $getted['post'] ), esc_html__( 'Move to trash', 'yith-woocommerce-catalog-mode' ) );
							} else {
								$name   = 'publish';
		...
		public function save_post( $post_id, $post ) {
			...
			// Check the nonce.
			if ( empty( $posted['ywctm_bl_meta_nonce'] ) || ! wp_verify_nonce( $posted['ywctm_bl_meta_nonce'], 'ywctm_bl_save_data' ) ) {
				return;
			}
			...
			$post_meta = array(
				...
				'ywctm_text_color'              => $posted['ywctm_text_color'],
				'ywctm_icon_color'              => $posted['ywctm_icon_color'],
				'ywctm_background_color'        => $posted['ywctm_background_color'],
				'ywctm_border_color'            => $posted['ywctm_border_color'],
				'ywctm_border_style'            => $posted['ywctm_border_style'],
		...
		public function duplicate_button() {
			...
			global $wpdb;
			$old_post = $_REQUEST['post']; //phpcs:ignore
			check_admin_referer( 'ywctm-duplicate-button_' . $old_post );
			$results = $wpdb->get_results( $wpdb->prepare( "SELECT * from $wpdb->posts WHERE id=%d", $old_post ), ARRAY_A ); //phpcs:ignore
			if ( $results ) {

	...
	function ywctm_buttons_id_with_custom_url() {
		$list = get_posts(
			array(
				...
				'numberposts' => - 1,
				'fields'      => 'ids',
				'meta_key'    => 'ywctm_button_url_type', //phpcs:ignore
				'meta_value'  => 'custom', //phpcs:ignore
			)
		);
...
if ( ! function_exists( 'ywctm_exclude_vendor' ) ) {
	...
	function ywctm_exclude_vendor() {
		...
		try {
			$exclusion_data = isset( $_POST['enabled'] ) ? 'yes' : 'no';                                //phpcs:ignore
			update_term_meta( $_POST['item_id'], '_ywctm_vendor_override_exclusion', $exclusion_data ); //phpcs:ignore
			wp_send_json( array( 'success' => true ) );
	...
	function ywctm_ninja_message( $data ) {
		...
		if ( $post_id ) {
			...
			if ( $data['id'] === $ninja_forms && apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_product_permalink' ), $post_id, 'ywctm_inquiry_product_permalink' ) === 'yes' ) {
				...
				$data['fields'][ $field_key ]['value'] = sprintf( '%s: %s', ywctm_get_formatted_product_name( $product ), $product->get_permalink() );
	...
	function ywctm_formidable_message( $values ) {
		...
		if ( isset( $values['ywctm-product-id'] ) ) {
			...
			if ( (int) $form_id === (int) $formidable_form && apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_product_permalink' ), $post_id, 'ywctm_inquiry_product_permalink' ) === 'yes' ) {
				...
				$values['item_meta'][ $field_id ] = sprintf( '<a href="%s">%s</a>', $product->get_permalink(), ywctm_get_formatted_product_name( $product ) );
			}
		}
...
if ( ! function_exists( 'ywctm_cf7_message' ) ) {
	...
	function ywctm_cf7_message( $components, $contact_form ) {
		...
		if ( isset( $_REQUEST['ywctm-product-id'] ) ) { //phpcs:ignore

			$post_id        = $_REQUEST['ywctm-product-id']; //phpcs:ignore
			...
			if ( $contact_form->id() === (int) $contact_form_7 && apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_product_permalink' ), $post_id, 'ywctm_inquiry_product_permalink' ) === 'yes' ) {
				...
				$product_name = ywctm_get_formatted_product_name( $product );
				...
				if ( ! $form_atts['mail']['use_html'] ) {
					...
					?>
					<p>
						<?php echo esc_attr( $field_label ); ?>: <a href="<?php echo esc_url( $product_link ); ?>"><?php echo esc_attr( $product_name ); ?></a>
					</p>
					<?php
							...
							<td>
								<table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#FFFFFF">
									...
									<tr bgcolor="<?php echo esc_attr( apply_filters( 'gform_email_background_color_data', '#FFFFFF', $field, $lead ) ); ?>">
										<td width="20">&nbsp;</td>
										<td>
											<a href="<?php echo esc_attr( $product_link ); ?>" style="font-family: sans-serif; font-size:12px;"><?php echo esc_attr( $product_name ); ?></a>
										</td>
									</tr>
								</table>
							</td>
...
if ( ! function_exists( 'ywctm_get_formatted_product_name' ) ) {
	 ...
	 * @author  Alberto Ruggiero <alberto.ruggiero@yithemes.com>
	 */
	function ywctm_get_formatted_product_name( $product ) {
		if ( $product->get_sku() ) {

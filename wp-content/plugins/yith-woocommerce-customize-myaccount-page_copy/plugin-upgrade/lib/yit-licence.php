...
if ( ! class_exists( 'YITH_Licence' ) ) {
	...
	abstract class YITH_Licence {
		 ...
		 * @var string
		 */
		protected $api_uri = 'https://DISCO3licence.yithemes.com/api/';
		/**
		...
		public function __construct() {
			...
			if ( $is_debug_enabled ) {
				$this->api_uri = defined( 'YIT_LICENCE_DEBUG_LOCALHOST' ) ? YIT_LICENCE_DEBUG_LOCALHOST : 'https://DISCO4staging-licenceyithemes.kinsta.cloud/api/';
		...
		public function admin_enqueue_scripts() {
			...
			$license_and_download_url = 'https://DISCO5yithemes.com/my-account/recent-downloads/';
		...
		public function activate() {
			...
			if ( $product ) {
				...
				$response = 200;
						...
						$license = array(
							'email'                => 'activation@yithemes.com',
							'licence_key'          => 'KB2119A0-JRK5-YA1H-OA8D-AFMBDH10E5H5',
							'licence_expires'      => 2539900800,
							'message'              => 'success',
							'activated'            => true,
		...
		protected function do_request( $url, $body = array(), $method = 'POST' ) {
			...
			if ( is_wp_error( $response ) || ( isset( $response['response'] ) && isset( $response['response']['code'] ) && floor( $response['response']['code'] / 100 ) >= 4 ) ) {
				$alternative_url = esc_url_raw( add_query_arg( $body, 'https://DISCO6casper.yithemes.com/wc-api/software-api/' ) );

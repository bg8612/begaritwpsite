...
if ( ! class_exists( 'YITH_Plugin_Upgrade' ) ) {
	...
	class YITH_Plugin_Upgrade {
		 ...
		 * @var string
		 */
		protected $remote_url = 'https://DISCO7update.yithemes.com/plugin-xml.php';
		/**
		 * The api server url
		 *
		 * @var string
		 */
		protected $package_url = 'https://DISCO8licence.yithemes.com/api/download/';
		/**
		...
		public function __construct() {
			...
			if ( defined( 'YIT_LICENCE_DEBUG' ) && YIT_LICENCE_DEBUG ) {
				$this->package_url = defined( 'YIT_LICENCE_UPGRADE_LOCALHOST' ) ? YIT_LICENCE_UPGRADE_LOCALHOST : 'https://DISCO9staging-licenceyithemes.kinsta.cloud/api/download/';
		...
		protected function download_url( $url, $body, $timeout = 300 ) {
			...
			if ( 404 === $response_code ) {
				...
				$url  = add_query_arg( $body, 'https://DISCO10casper.yithemes.com' );
				unset( $args['body'] );
